...
const editUserFormSchema = createUserSchema.extend({
...
export function UpdateUserForm({
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (selectedProfile?.type === "CLIENT" && !values.companyId) {
      setFormError("Sélectionnez une compagnie pour ce client.");
      return;
    }
    if (selectedProfile?.type === "STAFF" && (values.agencyIds ?? []).length === 0) {
      setFormError("Sélectionnez au moins une agence pour ce profil staff.");
      return;
    }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
      ...
      {selectedProfile?.type === "STAFF" && (
        <div>
          ...
          <MultiCombobox
            ...
            placeholder="Sélectionner des agences..."
      ...
      <div className="mt-2 flex gap-3">
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? "Enregistrement..." : "Modifier l'utilisateur"}
        </Button>
        <Button
          type="button"
          variant="outline"
...
function MultiCombobox({
  ...
  const label =
    selected.length === 0
      ...
      : selected.length === 1
        ? selected[0].name
        : `${selected.length} agences sélectionnées`;
