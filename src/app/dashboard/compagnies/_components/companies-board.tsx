...
import { Building2, Plus, Search, X, Edit, Trash2, MoreHorizontal } from "lucide-react";
...
type FormValues = z.input<typeof companyFormSchema>;
...
function CompanyModal({
  ...
  open: boolean;
  mode?: "create" | "edit";
  initialValue?: CompanyRow;
  onClose: () => void;
  onSubmitSuccess: (row: CompanyRow) => void;
}) {
    ...
    try {
      if (mode === "edit" && initialValue) {
        const res = await http.patch<{
          data: Omit<CompanyRow, "agenciesCount" | "usersCount">;
        }>(`/companies/${initialValue.id}`, values);
        onSubmitSuccess({
          ...res.data.data,
          agenciesCount: initialValue.agenciesCount,
          usersCount: initialValue.usersCount,
        });
      } else {
        const res = await http.post<{
          data: Omit<CompanyRow, "agenciesCount" | "usersCount">;
        }>("/companies", values);
        onSubmitSuccess({ ...res.data.data, agenciesCount: 0, usersCount: 0 });
      }
      reset();
      onClose();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-[1700] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="absolute inset-0 bg-black/55" onClick={isSubmitting ? undefined : onClose} aria-hidden />
      <form
      ...
      >
        ...
        <div className="pr-10">
          ...
          <h2 className="font-display text-xl font-semibold text-text">
            {mode === "create" ? "Ajouter une compagnie" : "Modifier la compagnie"}
          </h2>
          <p className="mt-1 text-sm leading-6 text-text-2">
            {mode === "create"
              ? "Renseignez les informations de la nouvelle compagnie."
        ...
        <div className="mt-6 grid gap-4">
          <div>
            <label className={label} htmlFor="company-name">Raison sociale</label>
            <Input id="company-name" placeholder="Ex. SANLAM" {...register("name")} />
            {errors.name && <p className={errCls}>{errors.name.message}</p>}
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className={label} htmlFor="company-phone">Contact</label>
              <Input id="company-phone" placeholder="Ex. 0196854785" {...register("phone")} />
              {errors.phone && <p className={errCls}>{errors.phone.message}</p>}
            </div>
            <div>
              <label className={label} htmlFor="company-address">Adresse</label>
              <Input id="company-address" placeholder="Ex. Agla" {...register("address")} />
...
export function CompaniesBoard({
  ...
  // Local state for rows and pagination
  const [rows, setRows] = useState<CompanyRow[]>(initialRows);
  ...
  // Modals state
  const [modal, setModal] = useState<{ mode: "create" | "edit"; company?: CompanyRow } | null>(null);
    ...
    async function loadData() {
      try {
        const res = await http.get<{
          data: CompanyRow[];
          meta: { total: number };
        }>(`/companies?page=${page}&limit=${pageSize}&q=${encodeURIComponent(debouncedQuery)}`);
  ...
  function handleCreatedOrUpdated(row: CompanyRow) {
    if (modal?.mode === "edit") {
      setRows((current) => current.map((r) => (r.id === row.id ? row : r)));
    } else {
      // Reload current page or insert optimistic row
  ...
  async function handleDeleteCompany() {
    ...
    setIsDeleting(true);
    try {
      await http.delete(`/companies/${confirmDelete.id}`);
    ...
    <div className="p-6 md:p-8">
      <div className="flex flex-col gap-4 border-b border-border pb-6 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-2 font-display text-3xl font-semibold text-text">Gestion des compagnies</h1>
          <p className="mt-2 text-sm text-text-2">Liste des compagnies enregistrées.</p>
        </div>
        {canCreate && (
          <Button type="button" onClick={() => setModal({ mode: "create" })} className="w-fit gap-2">
      ...
      <div className="mt-6">
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Rechercher par raison sociale, contact ou adresse..."
                ...
                {rows.map((row) => (
                  <tr key={row.id} className="transition-colors hover:bg-surface-2/60">
                    ...
                    <td className="px-4 py-4 text-right">
                      {canCreate && (
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-52">
                            <DropdownMenuItem onClick={() => setModal({ mode: "edit", company: row })}>
                      ...
                      {!canCreate && <span className="text-text-2">—</span>}
                    </td>
                  </tr>
                ))}
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-4 py-10 text-center text-sm text-text-2">
      ...
      <ConfirmDialog
        ...
        description={
          confirmDelete
            ? `Êtes-vous sûr de vouloir supprimer la compagnie "${confirmDelete.name}" ? Elle sera archivée (soft-delete).`
            : undefined
        }
